...
static bool provider_is_stepfun(void)
{
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (MIMI_SECRET_API_KEY[0] != '\0') {
        ...
        ESP_LOGI(TAG, "Loaded API key from secrets (len=%d)", (int)strlen(s_api_key));
    }
    ...
    if (s_api_key[0]) {
        ESP_LOGI(TAG, "LLM proxy initialized (provider: %s, model: %s, key_len=%d)", 
                 s_provider, s_model, (int)strlen(s_api_key));
    } else {
